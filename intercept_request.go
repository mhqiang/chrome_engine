...
func (tab *Tab) InterceptRequest(v *fetch.EventRequestPaused) {
	...
	if err != nil {
		logger.Debug("InterceptRequest parse url failed: ", err)
...
func (tab *Tab) HandleNavigationReq(req *model2.Request, v *fetch.EventRequestPaused) {
	...
	if tab.FoundRedirection && tab.IsTopFrame(v.FrameID.String()) {
		logger.Debug("redirect navigation req: " + req.URL.String())
		...
		logger.Debug("has redirection_flag: " + req.URL.String())
		...
		logger.Debug("main navigation req: " + navReq.URL.String())
		// 手动设置POST信息
