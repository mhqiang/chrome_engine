...
func NewTab(browser *Browser, navigateReq model2.Request, config TabConfig) *Tab {
	...
	chromedp.ListenTarget(*tab.Ctx, func(v interface{}) {
		...
		case *network.EventRequestWillBeSent:
			if string(v.RequestID) == string(v.LoaderID) &&
				v.Type == "Document" && tab.TopFrameId == "" {
...
func (tab *Tab) Start() {
	logger.Info("Crawling " + tab.NavigateReq.Method + " " + tab.NavigateReq.URL.String())
	defer tab.Cancel()
	if err := chromedp.Run(*tab.Ctx,
		...
		logger.Warn("navigate timeout ", tab.NavigateReq.URL.String())
	}
	...
	select {
	case <-tab.NavDone:
		logger.Debug("all navigation tasks done.")
...
func IsIgnoredByKeywordMatch(req model2.Request, IgnoreKeywords []string) bool {
	for _, _str := range IgnoreKeywords {
		if strings.Contains(req.URL.String(), _str) {
			logger.Info("ignore request: ", req.SimpleFormat())
			return true
		}
	}
	return false
